...
local function do_all_fold(dot_matrix, dot_list, fold_list)
    for _, fold in ipairs(fold_list)
    do
        do_fold(dot_matrix, dot_list, fold)
    end
end

local function print_matrix(dot_matrix, size)
    for i = 0, size, 1
    do
        for j = 0, size, 1
        do
...
do_all_fold(dot_matrix, dot_list, fold_list)
print_matrix(dot_matrix, 100)